...
void start_cmdio_alarm(void); //控制连接闹钟
...
void handle_alarm_timeout(int sig)
{
	shutdown(p_sess->ctrl_fd, SHUT_RD); //关闭读方向的半连接
	ftp_reply(p_sess, FTP_IDLE_TIMEOUT, "Timeout.");
	shutdown(p_sess->ctrl_fd, SHUT_WR);  //关闭写方向的半连接
	exit(EXIT_FAILURE); //也会导致nobody进程终止
}

void handle_sigalrm(int sig)  //数据连接发生超时
{
...
{
	ftp_reply(sess, FTP_GREET, "(syyftpd 0.1)");
	int ret;
	while (1) {
		...
		start_cmdio_alarm(); //启动闹钟
		ret = readline(sess->ctrl_fd, sess->cmdline, MAX_COMMAND_LINE);
		if (ret == -1)
			ERR_EXIT("readline");
		else if (ret == 0)  //检测到客户端退出，退出ftp服务进程
			exit(EXIT_SUCCESS);
...
void limit_rate(session_t *sess, int bytes_transfered, int is_upload)
{
	sess->data_process = 1;  //表示当前处于数据连接状态
...
void upload_common(session_t *sess, int is_append)
{
	...
	// 打开文件
	int fd = open(sess->arg, O_CREAT | O_WRONLY, 0666); //创建文件，可写，权限是(0666&UMSK）
	if (fd == -1) {
	...
	char buf[65536]; //这个位置可以设置，发送窗口大小，会影响传输速度
	...
	while (1) {
		...
		if (sess->abor_received) { //如果睡眠过程中收到ABOR命令
			flag = 2;
			break;
		}
...
int port_active(session_t *sess)
{
	if (sess->port_addr) {
		if (pasv_active(sess)) {
			fprintf(stderr, "both port an pasv are active"); //不能主动被动都激活
			exit(EXIT_FAILURE);
		}
		return 1;
	}
...
int get_port_fd(session_t *sess)  //获取主动模式数据连接套接字
{
	/*
	...
	*/
	...
	if (res == PRIV_SOCK_RESULT_BAD) { //如果创建数据连接失败
		return 0;
	}
...
static void do_cdup(session_t *sess)  //切换到上层路径
{
	if (chdir("..") < 0) {
		ftp_reply(sess, FTP_FILEFAIL, "Failed to change directory.");
		return;
...
static void do_retr(session_t *sess)
{
	...
	struct stat sbuf;
	ret = fstat(fd, &sbuf);
	if (!S_ISREG(sbuf.st_mode)) {  //判断是否普通文件
		ftp_reply(sess, FTP_FILEFAIL, "Failed to open file.");
		return;
	}

	if (offset != 0) {
		ret = lseek(fd, offset, SEEK_SET);  //设置文件偏移
		if (ret == -1) {
			ftp_reply(sess, FTP_FILEFAIL, "Failed to open file.");
			return;
		}
...
//150 Opening BINARY mode data connection 
	...
	while (bytes_to_send) {
		...
		ret = sendfile(sess->data_fd, fd, NULL, num_this_time);  //返回实际发送的字节数
		if (ret == -1) {
			flag = 2;
			break;
		}
...
static void do_list(session_t *sess)
{
	...
	// 传输列表
	list_common(sess, 1);
	// 关闭数据套接字，一定要关闭，否则客户端的数据连接会一直阻塞在接收服务器的数据
	close(sess->data_fd);
	sess->data_fd = -1;  //数据连接结束，数据连接是短暂的，每次数据传输完成都要关闭，控制连接一直存在
	// 226
...
static void do_rnfr(session_t *sess)  //将要被重命名的文件名
{
...
static void do_rnto(session_t *sess)  //重命名后的文件名
{
	...
	rename(sess->rnfr_name, sess->arg); //文件重命名
...
static void do_size(session_t *sess) //只能获取文件的大小，不能获取目录的大小
{
...
static void do_noop(session_t *sess)
{
	ftp_reply(sess, FTP_NOOPOK, "NOOP ok."); //客户端定期发送noop可以防止服务端控制链接空闲断开
