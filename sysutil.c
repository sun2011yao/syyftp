...
void send_fd(int sock_fd, int fd)  //通过UNIX域套接字发送文件描述符
{
	int ret;
	struct msghdr msg;
...
static int lock_internal(int fd, int lock_type)
{
	...
	the_lock.l_whence = SEEK_SET;  //加锁的位置，文件头部
	the_lock.l_start = 0;  //头部的偏移位置
	the_lock.l_len = 0;  //加锁的字节数，若为0是加锁整个文件
	do {
		ret = fcntl(fd, F_SETLKW, &the_lock);  //fcntl可用于加锁操作,F_SETLKW表示加锁失败阻塞
	}
	while (ret < 0 && errno == EINTR);
...
int unlock_file(int fd)
{
	...
	ret = fcntl(fd, F_SETLK, &the_lock); //F_SETLK表示无法解锁直接返回失败，不要阻塞
...
void nano_sleep(double seconds)
{
	...
	int ret;
	do {
		ret = nanosleep(&ts, &ts);  //若被中断，继续睡眠，ts会存睡眠剩余时间，循环睡眠
	}
...
{
	int ret;
	ret = fcntl(fd, F_SETOWN, getpid()); //F_SETOWN：设置将要在文件描述词fd上接收SIGIO 或 SIGURG事件信号的进程或进程组标识 。 
	if (ret == -1) {
		ERR_EXIT("fcntl");
	}
}
