...
char priv_sock_get_cmd(int fd)
{
	...
	if (ret == 0) {
		printf("ftp process exit\n");   //当ftp服务进程退出时，会关闭child_fd，nobody进程读取结果ret=0,知道子进程退出，也紧跟着退出，所以一个客户端退出会使ftp服务进程和NOBODY进程都退出
		exit(EXIT_SUCCESS);
	}
	if (ret != sizeof(res)) {
...
{
	priv_sock_send_int(fd, (int)len);  //字符串是不定长的，所以要首先发送一个长度，再实际发送字符串
...
{
	send_fd(sock_fd, fd);  //通过UNIX域套接字发送文件描述符
}
